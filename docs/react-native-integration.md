...
- Example helper (pseudo):
  function pcm16ToWav(base64Pcm, sampleRate = 24000) { /_ attach WAV header _/ }
...
const socket = io(`${API_BASE}/voice`, {
  auth: { token },
...
socket.on("voice:connected", ({ sessionId }) => {
...
socket.on("voice:assistant-transcript", (d) => {
  if (!d.responseId) {
    if (d.isFinal) addMessage(d.transcript);
    return;
  }
